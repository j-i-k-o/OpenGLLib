...
void main()
{
	...
	vec4 diffuse = light.diffuse*diffuseLighting;
	//specular
	...
	gl_FragColor = texcolor/3.0 + (ambient + diffuse + specular)*(diminish/(attenuation.constant+attenuation.linear*length(L-P)+attenuation.quadratic*length(L-P)*length(L-P)));
}
)"
